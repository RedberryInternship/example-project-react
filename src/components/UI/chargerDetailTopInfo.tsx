...
const chargerDetailTopInfo = ({chargerLocationDirectionPress,showChargerLocationPress, favouritePress, code, name, location, distance } : any) => {
  ...
  useEffect(() => {
    distance().then(({data} : any) =>{
      data.rows && data.rows[0] && data.rows[0].elements && data.rows[0].elements[0] && setDistance(data.rows[0].elements[0].distance.value)
    })
  }, [])
      ...
      <View style={styles.container}>
        <View style={{flexDirection:"row", justifyContent:"space-between",  flex:1}}>
          <View style={{flex:1, justifyContent:"space-between",}}>
            <Text style={{fontSize:13, color:"white", opacity:0.8}}>{ name }</Text>
            <Text style={{fontSize:15, color:"white", fontWeight:"bold"}} >კოდი:#{ code }</Text>
          </View>
          <View style={{}}>
            <TouchableOpacity 
        ...
        <View style={{flexDirection:"row", justifyContent:"space-between",  flex:1}}>
          ...
          <View style={{ justifyContent:"flex-end"}}>
            ...
            >  
              ...
              <Text style={{color : Colors.primaryWhite, fontSize:13, marginLeft:8, fontWeight:"bold"}} numberOfLines={1}>{_distance} {t("km")}</Text>
            </TouchableOpacity>
          </View>      
        </View>
      </View>
