    ...
    updatePulse = () => {
        if (this.mounted) {
            const pulses = this.state.pulses.map((p, i) => {
                ...
                let newDiameter = (p.diameter > maxDiameter ? this.props.initialDiameter : p.diameter + 1);
                let centerOffset = ( maxDiameter - newDiameter ) / 2;
                let opacity = Math.abs( ( (this.state.maxDiameter -newDiameter) / this.state.distance )  );
                let pulse = {
                    pulseKey: i + 1,
